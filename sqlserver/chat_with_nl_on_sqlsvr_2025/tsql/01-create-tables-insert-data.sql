...
-- Best for debugging, migrations, or multi‑model experimentation.
CREATE TABLE dbo.products_vector
(
    productid INT NOT NULL PRIMARY KEY,

    -- Cheap, fast, high‑recall embedding (MiniLM or similar)
    embedding_small VECTOR(384) NULL,

    -- Mid‑tier precision, optional: only populate when you need high‑precision ranking (OpenAI text-embedding-3-small)
    embedding_ranker VECTOR(1536) NULL,

    -- Highest precision, optional: only populate when you need maximum‑quality ranking (OpenAI text-embedding-3-large)
    -- embedding_large VECTOR(3072) NULL, but currently QL Server 2025 does NOT allow VECTOR dimensions larger than 1998
